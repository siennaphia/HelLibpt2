...
// Ciphertext<DCRTPoly> costFunction(const std::shared_ptr<CryptoContext<DCRTPoly>>& cryptoContext,
//                                   const std::shared_ptr<LPKeyPair<DCRTPoly>>& keyPair,
//                                   const Ciphertext<DCRTPoly>& predictions,
//                                   const Ciphertext<DCRTPoly>& trainOutput) 
// {

//     // Placeholder for a value 'a' - Not used in the original function
//     // auto a = cryptoContext->MakeCKKSPackedPlaintext({0.0});
//     // auto aEncrypted = cryptoContext->Encrypt(keyPair->publicKey, a);
//     auto a = 0;

//     auto b = cryptoContext->EvalSub(predictions, trainOutput);
//     auto copyB = cryptoContext->Decrypt(keyPair->secretKey, b);
//     auto copyBValues = copyB->GetRealPackedValue();
//     // copyB->SetLength(8); // Not directly applicable in C++, adjustment needed

//     std::cout << "copy_b: ";
//     for (auto& val : copyBValues) {
//         std::cout << val << " ";
//     }
//     std::cout << std::endl;

//     std::vector<double> newB;
//     for (auto& i : copyBValues) {
//         newB.push_back(static_cast<double>(i));
//     }
//     auto plaintext = cryptoContext->MakeCKKSPackedPlaintext(newB);
//     auto ciphertext = cryptoContext->Encrypt(keyPair->publicKey, plaintext);
//     auto newBEncrypted = ciphertext;

//     auto bmod = cryptoContext->EvalSquare(newBEncrypted);
//     auto result = cryptoContext->Decrypt(keyPair->secretKey, bmod);
//     // result->SetLength(8); // Adjust as needed

//     std::cout << "b^2 = ";
//     auto resultValues = result->GetRealPackedValue();
//     for (auto& val : resultValues) {
//         std::cout << val << " ";
//     }
//     std::cout << std::endl;

//     auto bCount = bmod; // Assuming 'bCount' should be initialized with 'bmod'
//     // Loop logic here might need adaptation based on specific use case

//     // Final computation
//     double cal = 1.0 / static_cast<double>(trainOutput.size()); // Adjust based on actual size or method
//     auto evalMult = cryptoContext->EvalMult(a, cal); // Assuming 'a' is correctly set up before
...
// int main() {
//     // Initialize the OpenFHE library
//     OpenFHE::Init();

//     // Setup parameters
//     usint cyclotomicOrder = 8192; // Example value, adjust based on needs
//     double scalingFactorBits = 40; // Example scaling factor
//     SecurityLevel securityLevel = HEStd_128_classic; // Example security level

//     // Create context
//     auto cc = CryptoContextFactory<DCRTPoly>::genCryptoContextCKKS(
//         cyclotomicOrder, securityLevel, scalingFactorBits,
//         RELIN, HYBRID, 0, OPTIMIZED, 2, 30, 60);
...
//     auto keys = cc->KeyGen();
//     cc->EvalMultKeyGen(keys.secretKey);
//     cc->EvalAtIndexKeyGen(keys.secretKey, {1, -1}); // For rotations, if needed

//     // Example plaintext vectors
//     std::vector<double> predictionsVec = {0.5, -0.25, 0.75};
//     std::vector<double> trainOutputVec = {0.6, -0.2, 0.8};

//     // Convert to CKKS plaintexts
//     auto predictionsPt = cc->MakeCKKSPackedPlaintext(predictionsVec);
//     auto trainOutputPt = cc->MakeCKKSPackedPlaintext(trainOutputVec);

//     // Encrypt plaintexts
//     auto predictionsCt = cc->Encrypt(keys.publicKey, predictionsPt);
//     auto trainOutputCt = cc->Encrypt(keys.publicKey, trainOutputPt);

//         // Call your function
//     auto resultCt = costFunction(cc, keys, predictionsCt, trainOutputCt);

//     // Decrypt the result
//     Plaintext resultPt;
//     cc->Decrypt(keys.secretKey, resultCt, &resultPt);

//     resultPt->SetLength(predictionsVec.size()); // Adjust based on your output size

//     std::cout << "Decrypted result: ";
//     for (auto val : resultPt->GetRealPackedValue()) {
//         std::cout << val << " ";
//     }
//     std::cout << std::endl;
